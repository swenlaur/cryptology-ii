...
In the lecture, we used an alternative route to get the same goal. First defined an algorithm $\ADB^*(y)$ as $\ADB(y, \infty)$ that always finds a discrete logarithm. 
Then we computed the expected runningtime of $\ADB^*$ and constructed a $2\tau$-time algorithm $\ADB_{2\tau}^*$ that fails with probability $\tfrac{1}{2}$ and repeated the runs of  $\ADB_{2\tau}^*$  until the failure probability is $2^{-n}$. Do you get the similar runningtime and why?
\end{exercise}
\begin{solution}
\end{solution}
\end{document}
